...
def set_state_indices_slices(physics):
    ...
    for key in physics.StateVariables:
        if isinstance(key.value, list):
            idx = []
            for key2 in physics.StateVariables:
                if key2.value in key.value:
                    idx += [physics.state_indices[key2.name]]
            physics.state_indices[key.name] = min(idx)
            physics.state_slices[key.name] = slice(min(idx), max(idx)+1)
        else:
...
class PhysicsBase(ABC):
    ...
    thermo: ThermoBase object
        thermodynamic relationships
    transport: TransportBase object
    ...
    def __init__(self, thermo=None, transport=None, NDIMS=0):
        '''
    ...
    def set_physical_params(self, **kwargs):
        '''
