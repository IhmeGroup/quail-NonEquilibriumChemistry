...
def triangulate(physics, x, var, triangles=None):
    '''
    ...
    -------
        ...
        xc: element centroid xy-coordinates [2]
        xp: element principal xy-coordinates [2]
    ...
    # Triangulation
    tris = tri.Triangulation(x[:, 0], x[:, 1], triangles=triangles)
...
def plot_2D_general(physics, x, xc, triangles, var_plot, **kwargs):
    '''
    This function plots 2D contours via a triangulation of the coordinates
    ...
    if "show_centroids" in kwargs:
        if kwargs["show_centroids"]:
            plt.scatter(xc[:, 0], xc[:, 1], s=4, color='k')


def plot_2D(physics, x, xc, triangles, var_plot, ylabel, regular_2D,
            equal_AR=False, **kwargs):
    '''
    This function plots 2D contours via a triangulation of the coordinates
    stored in x. This is appropriate for general domains, but is much slower
    than plot_2D_regular.
    ...
    if regular_2D:
        ...
        plot_2D_general(physics, x, xc, triangles, var_plot, **kwargs)
...
def plot_line_probe(mesh, physics, solver, var_name, xy1, xy2, num_pts=101,
    ...
    ''' Interpolation '''
    x, _, _ = get_sample_points(mesh, solver, physics, solver.basis, True)
...
def get_sample_points(mesh, solver, physics, basis, equidistant=True):
    ...
    triangles = None
    if ndims == 2:
        # Precompute the triangulation in reference space
        triangles = Delaunay(xref).simplices

    # Allocate
    num_pts = xref.shape[0]
    x = np.zeros([mesh.num_elems, num_pts, ndims])
    xc = np.zeros([mesh.num_elems, ndims])
    ...
    for elem_ID in range(mesh.num_elems):
        ...
        x[elem_ID, :, :] = xphys
        xc[elem_ID, :] = mesh_tools.ref_to_phys(mesh, elem_ID, basis.CENTROID)

    return x, xc, triangles
...
def plot_solution(mesh, physics, solver, var_name, plot_numerical=True,
    ...
    # Get sample points
    x, xc, triangles = get_sample_points(mesh, solver, physics, solver.basis,
                                         equidistant_pts)
    ...
    if ndims == 1:
        ...
        plot_2D(physics, x, xc, triangles, var_plot, ylabel, regular_2D,
                equal_AR, **kwargs)
